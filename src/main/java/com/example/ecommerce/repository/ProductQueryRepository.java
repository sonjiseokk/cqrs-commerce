...
public class ProductQueryRepository {
    ...
    // 여러 필터 적용된 검색 기능 + 페이징
    public Page<ProductDto.ProductSummary> search(ProductQuery.ListProducts query) {
        // 페이징 정보 추출
...
//                        inStockCond(query.getInStock()),
        ...
        // 2. images flat 조회
        List<ProductImage> productImages = queryFactory.select(productImage)
                .from(productImage)
                .where(productImage.product.id.in(productIds))
                .fetch();

        // 2-1. productId 기준으로 그룹화
    ...
    private BooleanExpression categoryIn(List<Long> categoryIds) {
        ...
        return product.categories.any().category.id.in(categoryIds);
    }
